...
"""
Sorti v1.1.0 — PyInstaller *onedir* spec.

The v1.0.0 onefile build produced a 366 MB Sorti.exe that extracted to %TEMP%
on every launch. This spec emits dist/Sorti/ (Sorti.exe + _internal/) which
Inno Setup compresses into Sorti-Setup-v1.1.0.exe (~120 MB) and which starts
instantly because nothing is unpacked at runtime.

    pyinstaller Sorti.onedir.spec --noconfirm
    # then:  ISCC.exe Sorti.iss
"""
...
webview_submodules = [
    sm for sm in collect_submodules('webview')
    if 'android' not in sm and 'gtk' not in sm and 'qt' not in sm and 'cocoa' not in sm
]
